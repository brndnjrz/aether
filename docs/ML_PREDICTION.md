...
Below that, **Prediction History** logs every prediction generated for the ticker (persisted to `storage/{TICKER}_predictions.jsonl`, so it survives across sessions): a time-series line of bull probability with markers colored by direction, plus a table of the last 15 predictions (direction, probability, confidence, expected move, model accuracy at that time) and a running count of bullish/bearish/neutral calls logged. The log currently records predictions but does not yet auto-verify them against what happened 5 days later — `actual_outcome`/`correct` fields exist in the schema for that but are not back-filled automatically.

A disclaimer banner is shown at the bottom of the tab regardless of state, restating the 52-58% typical accuracy range, the inability to predict news/earnings/macro shocks, and that position sizing should come from the Position Sizer on the Portfolio page, not from this signal.
